...
var workObject, aboutObject, processObject, blogObject;
...
window.onload = function(){
	...
	blogObject = new blogSetup("blog");
	blogObject.setSizeOfElements();
	blogObject.startPosition();

	workObject.setSiblings([aboutObject,processObject,blogObject]);
	aboutObject.setSiblings([workObject,processObject,blogObject]);
	processObject.setSiblings([aboutObject,workObject,blogObject]);
	blogObject.setSiblings([workObject,aboutObject,processObject]);

	objectList.push(workObject);
	objectList.push(aboutObject);
	objectList.push(processObject);
	objectList.push(blogObject);
	...
	$($("#globalNave").children()[0]).bind("click",function(event){
		...
		for(var o = 0; o<objectList.length; ++o){
			objectList[o].resetSize();
		}
	})

	setTimeout(function(){
		for(var o = 0; o<objectList.length; ++o){
			objectList[o].resetSize(true);
		}
		setTimeout(function(){
...
////////////////////
	...
	function loadBlog(){
		if(toLoadBlog){
			toLoadBlog = false;
			$("#blog-background").load('/bloglist/',function(){
...
// Blog object //
/////////////////
	function blogSetup(paerentID){
		var parentNode = $("#"+paerentID)[0];
		var backgroundElement = $("#blog-background")[0];
		var windowWidth;


		//determining it's position relative to the other elements
			var siblingSections = parentNode.parentNode.childNodes; //siblings can contain the total numbers
			var theCountofElement; //from the list of emelements from the top of the page...
			for(var a = 0; a < siblingSections.length; ++a){
				if(parentNode.id == siblingSections[a].id){
					theCountofElement = a;
				}
