...
(function() {
  ...
  nvtooltip.show = function(pos, content, gravity, dist, parentContainer, classes) {
    ...
    container.style.left = left+'px'; //Symon
...
nv.models.axis = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      switch (axis.orient()) {
        ...
        axisLabel.enter().append('text').attr('class', 'nv-axislabel')
          ...
          if (showMaxMin) {
            ...
            d3.transition(axisMaxMin)
                .attr('transform', function(d,i) {
                  return 'translate(' + ( (i>0) ? scale.range()[i] : (scale.range()[i] + 25)) + ',0)'
                });
          }
          break;
        case 'right':
      ...
      scale0 = scale.copy();
  		...
  		if (hideText) container.selectAll('text').classed('hidden', true); 
  ...
  //* method to hide text
  chart.hideText = function(_) {
    if (!arguments.length) return hideText;
    hideText = _;
    return chart;
  }
...
nv.models.legend = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
	  ...
	  seriesEnter.append('rect') //Symon: Changing the attributes of the legend items
	      .attr('width', 15)
	      .attr('height', 7)
...
nv.models.line = function() {
  ...
  var margin = {top: 0, right: 0, bottom: 0, left: 0}
	...
	, isFlip = function(d) { return d.flip; }
    ;
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      d3.transition(groups.exit())
          ...
          .remove();
      groups
          .attr('class', function(d,i) { return (d.key.replace(/\s+/g, '-').toLowerCase()) + ' nv-group nv-series-' + i })
          .classed('hover', function(d) { return d.hover })
      ...
      areaPaths.enter().append('path')
          ...
          .attr('d', function(d) {
            return d3.svg.area()
                ...
                .y1(function(d,i) { return y0( y.domain()[1] <= 0 ? y.domain()[0] >= 0 ? 0 : y.domain()[1-zz] : y.domain()[zz] ) })
      ...
      d3.transition(groups.exit().selectAll('path.nv-area'))
          .attr('d', function(d) {
            return d3.svg.area()
                ...
                .x(function(d,i) { return x0(getX(d,i)) })
                .y0(function(d,i) { return y0(getY(d,i)) })
                .y1(function(d,i) {if (d.flip) zz=1; return y0( y.domain()[1] <= 0 ? y.domain()[0] >= 0 ? 0 : y.domain()[1-zz] : y.domain()[zz] ) })
                //.y1(function(d,i) { return y0(0) }) //assuming 0 is within y domain.. may need to tweak this
                .apply(this, [d.values])
          });
...
nv.models.lineChart = function() {
	...
	function numberWithCommas(x) {
	    return Math.round(x*1000).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	}
  ...
  var margin = {top: 30, right: 20, bottom: 50, left: 60}
    ...
    , tooltips = true
    , tooltip = function(key, x, y, e, graph) {
        return '<h3>' + numberWithCommas(y) + '</h3>'
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var g = wrap.select('g').attr('transform', 'translate(0,15)');
      ...
      if (showLegend) {
        legend.width(700); //Symon

        g.select('.nv-legendWrap')
            .datum(data)
			.attr('transform', 'translate(-8,-70)') //Symon
            .call(legend);
      ...
      legend.dispatch.on('legendClick', function(d,i) { 
        ...
        selection.transition().call(chart);
		$('.nv-group path').filter(function() {
        	if (!($(this).attr('d'))) $(this).remove(); });
      });

	  legend.dispatch.on('legendMouseover', function(d, i) {
	      d3.select('#chart .nv-line g:first-child .' + (d.key.replace(/\s+/g, '-').toLowerCase())).attr('id', 'hover');
      });

      legend.dispatch.on('legendMouseout', function(d, i) {
		  d3.select('#chart .nv-line g:first-child .' + (d.key.replace(/\s+/g, '-').toLowerCase())).attr('id', null);
      });
  ...
  lines.dispatch.on('elementMouseover.tooltip', function(e) {
	if (tooltipPos.left) {
		    e.pos = [tooltipPos.left, tooltipPos.top];
	}
	e.pos = [e.pos[0] +  margin.left, e.pos[1] + margin.top];
    dispatch.tooltipShow(e);
  });
  ...
  chart.tooltipPos = function(_) {
    if (!arguments.length) return tooltipPos;
    tooltipPos.top    = typeof _.top    != 'undefined' ? _.top    : tooltipPos.top;
    tooltipPos.left   = typeof _.left   != 'undefined' ? _.left   : tooltipPos.left;
    return chart;
  };
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (showLegend) {
        ...
        g.select('.nv-legendWrap')
            .datum(data)
			.attr('transform', 'translate(460,-75)') //Symon
            .call(legend);
        ...
        /*g.select('.nv-legendWrap')
            .attr('class', 'translate(281, -95)');*/ //Symon
      }
